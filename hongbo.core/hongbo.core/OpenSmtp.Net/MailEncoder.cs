...
using System.Text;
	...
	internal class MailEncoder
	{
		...
		///
		///
		internal static string ConvertSubjectToQP(string s,string charset)
		{
			string tmpstr=Convert.ToBase64String(Encoding.GetEncoding("gb2312").GetBytes(s.ToCharArray()));
			return "=?" + charset + "?B?" + tmpstr + "?=";
		...
		internal static string ConvertToQP(string s)
		{
			string tmpstr=Convert.ToBase64String(Encoding.GetEncoding("gb2312").GetBytes(s.ToCharArray()));
			string tmpreturn="";
			//return tmpstr;
			if(tmpstr.Length<=76) return tmpstr;
			for(int i=0;i<tmpstr.Length;i=i+76)
				tmpreturn+=tmpstr.Substring(i,76<tmpstr.Length-i?76:tmpstr.Length-i)+"\r\n";
//			return tmpstr;
			return tmpreturn;
			...
			for(int sidx=0; sidx < s.Length; sidx++)
			{
				...
				{
//					sb.Append("=" + Convert.ToString((byte)ch, 16));
//					sb.Append(Convert.ToBase64String(source(sidx)));
				}
				else
				{
					sb.Append(ch);
...
//			return "=?" + charset + "?Q?" + ConvertToQP(s) + "?=";
			return "=?" + charset + "?B?" + ConvertToQP(s) + "?=";
////			return "=?" + charset + "?B?" + "aGVsbG/W0M7ExNg=" + "?=";

...
namespace hongbao.CollectionExtension
{
    ...
    public static class IEnumerableExtension
    {
        ...
        public static T[] ToArray<T>(this IEnumerable iEnum, Func<object,int, T> funcT)
        {
            ...
            int index = 0; 
            foreach (var x in iEnum) resultList.Add(funcT(x, index++));
        ...
        /// <param name="funcT"></param>
        /// <returns></returns>
        public static IEnumerable<T> Convert<T>(this IEnumerable iEnum, Func<object, int, T> funcT)
        {
            List<T> resultList = new List<T>();
            int index = 0;
        ...
        /// <param name="funcT"></param>
        /// <returns></returns>
        public static IEnumerable<T> Convert<T>(this IEnumerable iEnum, Func<object,  T> funcT)
        {
            List<T> resultList = new List<T>();
            foreach (var x in iEnum)
            {
        ...
        {
            List<T> resultList = new List<T>();
            foreach (var x in iEnum) resultList.Add(funcT(x));
            return resultList.ToArray(); 
        }
        ...
        public static Dictionary<TKey, TElement> ToDictionary<TSource, TKey, TElement>(this IEnumerable<TSource> source, Func<TSource, int, TKey> keySelector, Func<TSource, int, TElement> elementSelector)
        {
            ...
            {
                var key = keySelector(obj, index);
                var value = elementSelector(obj, index);
                index++;
                result[key] = value;
            }
            return result;
        ...
        /// <summary>
        /// 根据对象的一个属性进行比较最后返回对象的一个Distinct列表;
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <typeparam name="TTarget"></typeparam>
        /// <param name="source"></param>
